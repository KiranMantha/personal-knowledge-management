---
title: "JavaScript Prototypes, a Pocket Reference"
url: https://medium.com/p/d88f550ffce3
---

# JavaScript Prototypes, a Pocket Reference

[Original](https://medium.com/p/d88f550ffce3)

Member-only story

# JavaScript Prototypes, a Pocket Reference

[![AJ Meyghani](https://miro.medium.com/v2/resize:fill:64:64/1*SD8MhMmGOmXdyVVeHuLNvA@2x.jpeg)](/@ajmeyghani?source=post_page---byline--d88f550ffce3---------------------------------------)

[AJ Meyghani](/@ajmeyghani?source=post_page---byline--d88f550ffce3---------------------------------------)
...
* Property and method look up follows the prototype chain. That is, if you want to look up a property in `a`, JavaScript will first look at `a` itself. If it cannot find it there, it will then look at `b`. If it can't find it in `b`, then it will look in `m`, and eventually if it can't find it in `m`, it will return `undefined` because `m` is the 'mother object' and is not linked to any other object.
* But if you want to look up a property in `b`, JavaScript will not look into `a`. It will follow the link direction and if it can't find it in `b`, it will look in `m`, and will return `undefined` if it cannot find it in `m`.
...
When you create any object, behind the scenes JavaScript links your object to the ‘mother object’ automatically:
...
And that’s why you can call the method on `str` because it is an object for a short period of time until it is garbage collected. Now, that we know what happens behind the scenes, it might be easier to answer the second question: where does the `replace` method come from? The answer is, it comes from `String.prototype` and if you think about it, the answer is consistent to what we learned in the previous section. And since JavaScript is dynamic, you can add a method to `String.prototype` object and magically all the pre existing strings in your code, will have a new method. This is not generally recommended, but let's demonstrate this by adding a new method on `String.prototype` called `first` that returns the first character of a given string:
...
As you can see, this mechanism is consistent across JavaScript, that is, for anything out there in JavaScript, there is a constructor function that has a prototype property that contains the methods and properties to be used by all the instances. If you understand this concept, you will understand the core of JavaScript and how it works. Below are some more objects with their corresponding prototype chains:
...
You can mimic the classical inheritance model in JavaScript with prototype objects. We are just going to demonstrate that here and we won’t dive into it since the classical inheritance model does not really fit JavaScript but we are just going to show that here because many seem to use this pattern. After that we will demonstrate functional mixins which are much more flexible and dynamic way of achieving inheritance in JavaScript.
...
Another important thing to remember is using the `new` keyword. Almost in all cases, you want to hide that away and not force the consumers of your API to use `new` to create instances:
...
By doing that, consumers won’t have to call the constructor with `new` which eliminates the possibility of `this` to be bound to the global object. The other important thing to note is how `this` is bound when the function is invoked with the `new` keyword. When the constructor function is invoke with the `new` keyword, the context object, `this` always refers to the instance. In the example above, we assign the name property of the instance to what is passed. Each instance will get their own names but they will all share the same prototype that enables them to efficiently call methods without the need of copying methods to each instance.
...
### Functional Mixins Over Inheritance
...
The `class` keyword was introduced in ES2015 and is a syntactic sugar for constructor functions. It should not confuse you, JavaScript does not have classes, it's merely for convenience and standardizing inheritance. Let's look at an example:
...
The code above, for all intents and purposes, is equivalent to the following:
...
We are not going to talk a lot about classes here, since learning classes is a matter of learning the syntax. It’s more important to learn the prototype concept than the class syntax. If you would like to learn more about classes, you can read more about them on [Mozilla Developer Network](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes).

### [*Next up: JavaScript Interview Questions, Functions and Prototypes*](/p/1145e3763bce)