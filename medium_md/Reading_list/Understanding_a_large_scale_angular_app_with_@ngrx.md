...
url: https://medium.com/p/80f9fc5660cc
---

# Understanding a large scale angular app with @ngrx

[Original](https://medium.com/p/80f9fc5660cc)

# Understanding a large scale angular app with [@ngrx](/u/5df0f4e36e72?source=post_page---user_mention--80f9fc5660cc---------------------------------------)

[![Kristy Johnstone](https://miro.medium.com/v2/resize:fill:64:64/0*Vx60ej5d7SuzwGiD.)](/@johnstonekristy?source=post_page---byline--80f9fc5660cc---------------------------------------)

[Kristy Johnstone](/@johnstonekristy?source=post_page---byline--80f9fc5660cc---------------------------------------)
...
A new `search action` is dispatched and the reducertakes the previous search state and updates it to the new state with the latest search info. In this example this is done by the `search reducer`which is specifically in charge of updating the search part of the stored state with the new data.
...
The store’s **select** methodknows how to fetch the current state and it sets up a subscription to the state changes using observables which allows the component to stay up to date with the latest changes in the store. A **selector** describes what specifically needs to be fetched from the store. A **selector** can be set up to describe single slices of the state or can group together different slices of the state to get the information that an individual component needs.
...
Being new to working on an app at such a large scale it is definitely appreciated when there is as little ‘magic’ happening as possible and once more fully understood, this is what @ngrx/store helps provide — predictable state in a manageable format.