...
url: https://medium.com/p/c1b2a57455af
---

# Self-Parking Car in <500 Lines of Code

[Original](https://medium.com/p/c1b2a57455af)
...
[![Oleksii Trekhleb](https://miro.medium.com/v2/resize:fill:64:64/1*Wta1U5y3Q7wJ7EC_iEq7Nw.jpeg)](/@trekhleb?source=post_page---byline--c1b2a57455af---------------------------------------)

[Oleksii Trekhleb](/@trekhleb?source=post_page---byline--c1b2a57455af---------------------------------------)
...
For example, the brain may send the signal with the value of `+1` to the engine muscle and it will start moving the car forward. The signal `-1` to the engine moves the car backward. At the same time, if the brain will send the signal `-1` to the steering wheel muscle, it will turn the car to the left, etc.
...
As input from the sensors, every `100ms` the brain will be getting `8` float numbers, each one in the range of `[0...4]`. For example, the input might look like this:
...
We may implement the generic polynomial function in the following way ([check this version of the article for better source-code formatting](https://trekhleb.dev/blog/2021/self-parking-car-evolution/)):
...
The output of a `linearPolynomial()` function is a float number. The `brainToMuscleSignal()` function needs to convert the wide range of floats to three particular integers, and it will do it in two steps:
...
To do the first part of the conversion we need to introduce a [Sigmoid Function](https://en.wikipedia.org/wiki/Sigmoid_function) that implements the following formula:
...
The implementation of the two conversion steps mentioned above would look like this:
